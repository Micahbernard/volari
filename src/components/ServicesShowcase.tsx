...
// doctrinal text in Geist Mono. On approach, the mercury
// surface distorts — RGB split + skew, like light through
// a prism hitting liquid metal.
//
// The scroll is a pinned horizontal rail driven by GSAP
// ScrollTrigger. Cards enter from opacity 0.4, scale 0.92,
// bloom to full presence at center, then fade as they exit.
// ─────────────────────────────────────────────────────────────
    ...
    <section ref={sectionRef} className="relative" id="services">
      ...
      <div ref={headingRef} className="px-6 pb-20 pt-32 md:px-10">
        <div className="mx-auto max-w-[90rem]">
          {/* Alchemical index number */}
          <span
            data-animate
            className="mb-5 block font-[family-name:var(--font-geist-mono)] text-[10px] uppercase tracking-[0.5em] text-v-accent/70 opacity-0"
          >
            004 — The Five Seals
          </span>

          {/* Main heading with decorative rules */}
          <div className="flex items-center gap-6 md:gap-10">
            <div
              data-animate
              className="hidden h-px w-16 bg-gradient-to-r from-v-accent/40 to-transparent opacity-0 md:block"
            />
            <h2
              data-animate
              className="font-[family-name:var(--font-playfair)] text-[clamp(2rem,5vw,4.5rem)] leading-[1.05] tracking-[-0.02em] text-v-chalk opacity-0"
            >
              What we{" "}
              <span className="italic text-v-accent">architect</span>
            </h2>
            <div
              data-animate
              className="hidden h-px flex-1 max-w-xs bg-gradient-to-r from-transparent via-v-smoke/20 to-transparent opacity-0 md:block"
            />
          </div>
          ...
          <p
            data-animate
            className="mt-5 max-w-lg font-[family-name:var(--font-geist-mono)] text-[11px] leading-[1.9] tracking-[0.02em] text-v-silver/70 opacity-0 md:text-xs"
          >
      ...
      <div
        ref={trackRef}
        className="flex items-stretch gap-6 px-6 pb-24 will-change-transform md:gap-10 md:px-10"
      >
        {SERVICES.map((service, i) => (
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-sm border border-v-smoke/15">
              <div
                data-card-visual
                className={`relative aspect-[16/10] w-full bg-gradient-to-br ${service.metal}`}
                style={{ willChange: "transform" }}
              >
                {/* Subtle radial sheen — like light on mercury */}
                <div
                  className="absolute inset-0"
                  style={{
                    background: `radial-gradient(ellipse 100% 80% at 30% 50%, ${service.bokeh}, transparent 70%)`,
                  }}
                />
                <div
                  className="absolute inset-0"
                  style={{
                    background: `radial-gradient(ellipse 80% 60% at 70% 70%, ${service.bokeh}, transparent 60%)`,
                  }}
                />
                ...
                {/* Large Roman seal — prominent, like an alchemical symbol */}
                <span
                  className="absolute bottom-5 right-6 font-[family-name:var(--font-playfair)] text-[clamp(4rem,10vw,8rem)] leading-none md:bottom-8 md:right-10"
                  style={{
                    background: "linear-gradient(135deg, var(--hero-metal-4), var(--hero-metal-6), var(--hero-metal-3))",
                ...
                <div
                  data-card-overlay
                  className="pointer-events-none absolute inset-0 flex items-center justify-center bg-v-black/25 opacity-0 backdrop-blur-[2px]"
                >
            ...
            {/* ── Card metadata — aligned with intention ── */}
            <div className="mt-7 flex flex-1 flex-col md:mt-9">
              ...
              {/* Notations — alchemical tags */}
              <div className="mt-auto flex flex-wrap gap-2 pt-7">
                {service.notations.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-v-smoke/25 px-3 py-1 font-[family-name:var(--font-geist-mono)] text-[9px] uppercase tracking-[0.2em] text-v-silver/60 transition-all duration-300 group-hover:border-v-accent/20 group-hover:text-v-silver/80"
                  >
                    {tag}
                  </span>
                ))}
